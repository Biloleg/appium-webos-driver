  ...
  async connect() {
    ...
    this.#ws?.on(WsEvent.ERROR, (err) => {
      ...
      if (err.message?.includes('ECONNRESET')) {
        this.#log.warn('Connection was reset by the TV. The TV may have been turned off, restarted, or the network connection was interrupted.');
      }
    });
  ...
  async #receiveResponse() {
    return await new B((res, rej) => {
      ...
      const onError = (err) => {
        ...
        if (err.message?.includes('ECONNRESET')) {
          rej(new Error('Connection reset by TV. Please check if the TV is on and network connection is stable.'));
        } else {
          rej(err);
        }
      };
  ...
  async #sendMessage(msgObj) {
    if (!this.isConnected()) {
      ...
      try {
        ...
        throw new Error(
          'Failed to reconnect socket WebSocket: ' + err.message + '. ' +
    ...
    try {
      ...
      if (err.message?.includes('ECONNRESET')) {
        throw new Error('Failed to send command: Connection reset by TV. Please check if the TV is on and network connection is stable.');
      }
      throw err;
    }
