...
export class WebOSDriver extends BaseDriver {
  ...
  async createSession(w3cCaps1, w3cCaps2, w3cCaps3) {
    ...
    try {
              log.info(`!!!!!!!Device Info retrieval called for device: ${deviceName}`);

      caps.deviceInfo = await getDeviceInfo(deviceName);
              log.info(`!!!!!!!Device Info: ${JSON.stringify(caps.deviceInfo)}`);
    ...
    log.info(`!!!!!!!Creating ValueBox and LGWSClient`);
    this.valueBox = ValueBox.create('appium-lg-webos-driver');
    log.info(`!!!!!!!Creating LGWSClient`);
    log.info(`!!!!!!!Device Host: ${deviceHost}, WS Ports: ${websocketPort}, ${websocketPortSecure}`);
  ...
  /**
   * Find element - intercepts to fix invalid locator strategies
   * @param {string} strategy - Locator strategy
   * @param {string} selector - Locator value
   * @returns {Promise<any>}
   */
   ...
   */
  async getPageSource() {
    log.info(`!!!!!!!!! [getPageSource] Getting page source and injecting attributes`);
    
    // First get the raw HTML from Chromedriver
    // @ts-ignore
    const rawSource = /** @type {string} */ (await this.#chromedriver.sendCommand('/source', 'GET'));
    log.info(`!!!!!!!!! [getPageSource] Got raw source, length: ${rawSource?.length || 0}`);
    ...
    const script = `
      ...
      console.log('!!!!!!!!! [getPageSource Script] Injected attributes into ' + count + ' elements');
      return count;
    `;
    
    try {
      const count = await this.#executeChromedriverScript('/execute/sync', script, []);
      log.info(`!!!!!!!!! [getPageSource] Injected attributes into ${count} elements`);
    } catch (error) {
      log.error(`!!!!!!!!! [getPageSource] Error injecting attributes:`, error);
    }
    
    // Now get the modified source
    // @ts-ignore
    const modifiedSource = /** @type {string} */ (await this.#chromedriver.sendCommand('/source', 'GET'));
    log.info(`!!!!!!!!! [getPageSource] Returning modified source, length: ${modifiedSource?.length || 0}`);
  ...
  async getText(elementId) {
    log.info(`!!!!!!!!! [getText] Getting text for element ${elementId} via JS execution`);
    const script = `
    ...
    try {
      ...
      log.info(`!!!!!!!!! [getText] Result:`, JSON.stringify(result));
      return /** @type {string} */ (result);
    } catch (error) {
      log.error(`!!!!!!!!! [getText] Error:`, error);
      throw error;
    }
  }
  ...
  async getElementRect(elementId) {
    log.info(`!!!!!!!!! [getElementRect] Getting rect for element ${elementId} via JS execution`);
    const script = `
      var element = arguments[0];
      var rect = element.getBoundingClientRect();
      return {
    ...
    try {
      ...
      log.info(`!!!!!!!!! [getElementRect] Result:`, JSON.stringify(result));
      const rectValue = /** @type {{x: number, y: number, width: number, height: number}} */ (result);
      log.info(`!!!!!!!!! [getElementRect] Returning:`, JSON.stringify(rectValue));
      return rectValue;
    } catch (error) {
      log.error(`!!!!!!!!! [getElementRect] Error:`, error);
      throw error;
    }
  }
  ...
  async getElementInfo(elementId) {
    log.info(`!!!!!!!!! [getElementInfo] Getting comprehensive info for element ${elementId}`);
    const script = `
      var element = arguments[0];
      var rect = element.getBoundingClientRect();
      var style = window.getComputedStyle(element);
      ...
      console.log('!!!!!!!!! [getElementInfo Script] Original attributes count:', element.attributes ? element.attributes.length : 0);
      console.log('!!!!!!!!! [getElementInfo Script] Attributes:', attributes);
      if (!element.attributes || element.attributes.length === 0) {
        console.log('!!!!!!!!! [getElementInfo Script] No original attributes, creating fallback');
        ...
        console.log('!!!!!!!!! [getElementInfo Script] Fallback attributes:', attributes);
      }
      return {
    ...
    try {
      ...
      log.info(`!!!!!!!!! [getElementInfo] Retrieved comprehensive element info with ${Object.keys(resultData.attributes || {}).length} attributes`);
      log.info(`!!!!!!!!! [getElementInfo] Attributes:`, JSON.stringify(resultData.attributes));
      return result;
    } catch (error) {
      log.error(`!!!!!!!!! [getElementInfo] Error:`, error);
      throw error;
    }
  }
  ...
  async getAttribute(name, elementId) {
    log.info(`!!!!!!!!! [getAttribute] Getting attribute "${name}" for element ${elementId}`);
    ...
    if (dimensionalAttrs.includes(name.toLowerCase())) {
      log.info(`!!!!!!!!! [getAttribute] Attribute "${name}" is dimensional - will compute from rect if not found`);
      ...
      if (attrResult !== null && attrResult !== '') {
        log.info(`!!!!!!!!! [getAttribute] Found actual attribute "${name}": ${attrResult}`);
        return /** @type {string|null} */ (attrResult);
      }
      
      // Otherwise, compute from bounding rect as a number
      log.info(`!!!!!!!!! [getAttribute] Attribute "${name}" not found or empty, computing from getBoundingClientRect`);
      const rectScript = `
        var element = arguments[0];
        var rect = element.getBoundingClientRect();
      ...
      log.info(`!!!!!!!!! [getAttribute] Computed "${name}" from rect: ${computedValue}`);
      return /** @type {number|null} */ (computedValue);
    }
    
    // For non-dimensional attributes, just return the attribute value
    log.info(`!!!!!!!!! [getAttribute] Getting regular attribute "${name}"`);
    const script = `
      var element = arguments[0];
      var attrName = arguments[1];
      return element.getAttribute(attrName);
    ...
    log.info(`!!!!!!!!! [getAttribute] Result for "${name}": ${result}`);
  ...
  async getName(elementId) {
    log.info(`!!!!!!!!! [getName] Getting tag name for element ${elementId}`);
    const script = `
      var element = arguments[0];
    ...
    log.info(`!!!!!!!!! [getName] Result: ${result}`);
    return /** @type {string} */ (result);
  }
  ...
  async getProperty(name, elementId) {
    log.info(`!!!!!!!!! [getProperty] Getting property "${name}" for element ${elementId}`);
    const script = `
      var element = arguments[0];
    ...
    log.info(`!!!!!!!!! [getProperty] Property "${name}" value: ${result}`);
    return result;
  }
  ...
  async elementEnabled(elementId) {
    log.info(`!!!!!!!!! [elementEnabled] Checking if element ${elementId} is enabled`);
    const script = `
      var element = arguments[0];
    ...
    log.info(`!!!!!!!!! [elementEnabled] Result: ${result}`);
    return /** @type {boolean} */ (result);
  }
  ...
  async elementSelected(elementId) {
    log.info(`!!!!!!!!! [elementSelected] Checking if element ${elementId} is selected`);
    const script = `
      var element = arguments[0];
    ...
    log.info(`!!!!!!!!! [elementSelected] Result: ${result}`);
    return /** @type {boolean} */ (result);
  }
  ...
  async getCssProperty(propertyName, elementId) {
    log.info(`!!!!!!!!! [getCssProperty] Getting CSS property "${propertyName}" for element ${elementId}`);
    const script = `
      var element = arguments[0];
      var propertyName = arguments[1];
      var style = window.getComputedStyle(element);
    ...
    log.info(`!!!!!!!!! [getCssProperty] Result for "${propertyName}": ${result}`);
    return /** @type {string} */ (result);
  }
