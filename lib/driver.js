...
export class WebOSDriver extends BaseDriver {
   ...
   * @returns {Promise<object|null>} Returns element info (with focused=true) or null if no focused element
   */
  async getFocusedElement() {
    // First, refresh element attributes to get the current focused state
    await this.injectElementAttributes();
    ...
    try {
      const focusedInfo = await this.#executeChromedriverScript('/execute/sync', script, []);
      return focusedInfo;
    } catch (error) {
  ...
  async injectElementAttributes() {
    const script = `
      ...
      Array.from(allElements).forEach(function(el) {
        ...
        // Set focused attribute - true for currently focused element, false otherwise
