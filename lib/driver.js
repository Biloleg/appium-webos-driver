...
export class WebOSDriver extends BaseDriver {
  ...
  #isChromedriverAutodownloadEnabled() {
    // Check both with and without driver prefix for Appium 2.x and 3.x compatibility
    if (this.isFeatureEnabled(CHROMEDRIVER_AUTODOWNLOAD_FEATURE) || 
        this.isFeatureEnabled(`webos:${CHROMEDRIVER_AUTODOWNLOAD_FEATURE}`)) {
      return true;
    }
    this.log.debug(
      `Automated Chromedriver download is disabled. ` +
        `Use '${CHROMEDRIVER_AUTODOWNLOAD_FEATURE}' or 'webos:${CHROMEDRIVER_AUTODOWNLOAD_FEATURE}' server feature to enable it`,
    );
    return false;
  }
